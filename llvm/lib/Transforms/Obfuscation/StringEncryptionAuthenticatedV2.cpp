//===- StringEncryptionAuthenticatedV2.cpp - authenticated string records ===//
...
public:
  ...
  // ObfuscationPassManager executes module passes before its function passes.
  // Delay string rewriting until finalization so the authenticated runtime is
  // not subsequently flattened, indirected, or constant-rewritten.
...
bool AuthenticatedStringEncryption::buildEncryptedTable(Module &M) {
  ...
  for (std::unique_ptr<StringRecord> &Owned : Records) {
    ...
    if (SizeOverflow || LimitExceeded) {
      ...
      return reject("authenticated string table exceeds its configured or addressable size");
    }
